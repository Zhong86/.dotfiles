...
readarray -t list < <(awk -F',' '{ for(i=1; i<NF; i++) { gsub(/^[ \t]+|[ \t]+$/, "", $i); print $i } }' "$text")

arrLen=${#list[@]}
ran=$(( RANDOM % arrLen ))
ranPoke="${list[ran]}"

readarray -t pokecmd < <(awk -v str="$ranPoke" 'BEGIN{ n=split(str, a, "_"); for(i=1; i<=n; i++) print a[i] }')
